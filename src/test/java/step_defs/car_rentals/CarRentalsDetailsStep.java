...
public class CarRentalsDetailsStep extends BaseStep {
    ...
    public void theUserValidatesThatPriceAndOfDisplayedCarInDetail_pageMatchesWithTheSelectedCarInFiltering_page(
        ...
        LOGGER.debug("The user validates that " + pick_up_location + "of displayed car in detail_page matches with the selected car in filtering_page");
        ...
        LOGGER.debug("The user validates that Price of displayed car in detail_page matches with the selected car in filtering_page");

        then(PAGES.getCarRentalsDetailsPage().getTransmissionOfSelectedCar()).isEqualTo(transmission);
        LOGGER.debug("The user validates that " + transmission + "of displayed car in detail_page matches with the selected car in filtering_page");

        then(PAGES.getCarRentalsDetailsPage().getCategoryOfTheSelectedCar()).isEqualTo(car_category);
        LOGGER.debug("The user validates that " + car_category + "of displayed car in detail_page matches with the selected car in filtering_page");
