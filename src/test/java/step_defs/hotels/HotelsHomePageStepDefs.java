...
public class HotelsHomePageStepDefs extends BaseStep {
    ...
    public void the_user_should_see_search_hotels_button() {
        then(PAGES.getHotelsHomePage().isVisibleSearchHotelButton()).withFailMessage("Search Hotels button is not displayed")
                .isTrue();
    }
    ...
    public void theUserShouldBeOnTheHotelSHomePage() {
        BrowserUtils.wait(1);
        then(PAGES.getHotelsHomePage().getStaysHeader()).withFailMessage("The user is not on the Stays Home Page").isTrue();
    }
