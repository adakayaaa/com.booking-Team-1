...
public class FlightsBookingPage extends BaseStep {
	...
	public void theUserShouldSeeThatText(String titleOfBookingPage) {
		then(PAGES.getFlightsBookingPage().getTitleOfBookingPage())
	...
	public void theUserShouldSeeThatOnlyOnTheTickets(String tripType) {
		then(PAGES.getFlightsBookingPage().getTextOfTripType()).withFailMessage("The trip type is wrong!")
			.isEqualTo(tripType);
	...
	public void theUserShouldSeeAlertMessage() {
		...
		then(alert.getText()).withFailMessage("The user shouldn't click select extra button")
			.isEqualTo("Please fill the following fields:");
