...
import pages.hotels.used_class.Hotel;
...
public class HotelsFilteringPage extends BasePage {
	...
	public boolean arePricesOfHotelsBetweenMinAndMax(int min, int max) {
		List<Integer> prices = pricesOfHotels.stream()
	...
	public Hotel clickOnHighestRatedHotel() {
		...
		String nameOfHotel = ratingsOfHotels.get(list.indexOf(max))
			.findElement(By.xpath("../../../div[1]/h1"))
			.getText();
		actions.moveToElement(ratingsOfHotels.get(list.indexOf(max)).findElement(By.xpath("../../div[2]/button")))
			.perform();
	...
	public Hotel clickOnHotelsSeeAvailabiliyButton(int numberOfHotel) {
		...
		int priceOfHotel = Integer.parseInt(seeAvailabilityButtons.get(numberOfHotel - 1)
			.findElement(By.xpath("../span[1]"))
			.getText()
			.substring(1));
		String nameOfHotel = seeAvailabilityButtons.get(numberOfHotel - 1)
			.findElement(By.xpath("../../../div[1]/h1"))
			.getText();
